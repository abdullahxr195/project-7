...
return res.status(400).json({message:"please enter product name and stocks and category name"})

}

if(price <= 0 || stock <= 0){

 return res.status(400).json({message:"product price and stock must be positive number"})
...
if(!isCatExist){
    ...
    return res.status(404).json({message:"the selected category is not find !"})

}

const product = await Product.creatr({ name, description,stock,price,image, catId})
if(!product){
...
return res.status(200).json({message:"this category has no products yet", products:[]})
